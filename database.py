...
async def init_db():
  ...
  try:
    ...
    # Таблица пользователей и языков (используем telegram_id вместо user_id)
    cur.execute("""
    ...
    cur.execute(
        "ALTER TABLE sponsors ADD COLUMN IF NOT EXISTS gender VARCHAR(10);"
    )
    cur.execute(
        "ALTER TABLE sponsor_drafts ADD COLUMN IF NOT EXISTS gender"
        " VARCHAR(10);"
    )
    cur.execute(
        "ALTER TABLE sponsors ADD COLUMN IF NOT EXISTS program_info TEXT;"
    )
    cur.execute(
        "ALTER TABLE sponsor_drafts ADD COLUMN IF NOT EXISTS program_info TEXT;"
    )
    cur.execute(
...
# Функции языка (исправлено на telegram_id)
def _get_user_language_sync(user_id: int) -> str:
  try:
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor()
    cur.execute(
        "SELECT language FROM users WHERE telegram_id = %s", (user_id,)
    )
    row = cur.fetchone()
    cur.close()
    conn.close()
...
def _set_user_language_sync(user_id: int, lang: str):
  try:
    ...
    cur.execute(
        """
            INSERT INTO users (telegram_id, language) VALUES (%s, %s)
...
def get_main_menu_keyboard(lang: str = "ru") -> ReplyKeyboardMarkup:
  if lang == "kk":
    keyboard = [
        [
            KeyboardButton(text="➕ Демеуші болу"),
            KeyboardButton(text="🤝 Демеушілер"),
        ],
        [
            KeyboardButton(text="🗓 Кесте"),
            KeyboardButton(text="📖 Күнделікті ой-толғаулар"),
        ],
        [
            KeyboardButton(text="? Көмек"),
            KeyboardButton(text="🌐 Тіл: Қазақша"),
        ],
    ]
  else:
    keyboard = [
...
def _add_message_sync(user_id: int, role: str, content: str):
  try:
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO dialog_history (user_id, role, content) VALUES (%s, %s,"
        " %s)",
...
def _get_recent_history_sync(user_id: int, limit: int = 6) -> list:
  try:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT role, content FROM dialog_history WHERE user_id = %s ORDER BY"
        " id DESC LIMIT %s",
        (user_id, limit),
    )
    rows = cur.fetchall()
