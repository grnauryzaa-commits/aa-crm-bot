...
async def ask_ai_for_beginner(user_id: int, user_message: str) -> str:
    ...
    if canonical_text:
        context_instruction = (
            ...
            f"Пользователь спрашивает о конкретном принципе. Ты ОБЯЗАН начать свой ответ с ДОСЛОВНОГО цитирования этого канона:\n"
            f"\"{canonical_text}\"\n"
            f"После цитаты ты можешь добавить 1-2 предложения от себя в стиле старшего товарища, но САМ ТЕКСТ КАНОНА менять запрещено!"
        )
    ...
    system_prompt = (
        ...
        "Твоя задача — давать абсолютно точные и правдивые ответы по программе АА.\n\n"
        "ПРАВИЛА:\n"
        "1. Если в запросе сработал канон, ты ДОЛЖЕН привести его точную формулировку (без выдумывания и искажения фактов).\n"
        "2. Говори тепло, по-дружески, без менторства.\n"
        "3. Никогда не признавайся, что ты бот или искусственный интеллект.\n"
        "4. Напоминай про кнопку «👤 Позвать живого служащего» под сообщением и сайт aaorg.kz."
    ...
    payload = {
        ...
        "messages": messages,
        "temperature": 0.01,  # Абсолютный минимум, чтобы убрать импровизацию
        "max_tokens": 600
    }
