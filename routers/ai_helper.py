...
async def ask_ai_for_beginner(user_message: str) -> str:
    ...
    system_prompt = (
        ...
        "2. Длина ответов: Отвечай емко и тепло (максимум 3–5 предложений). Если просят перечислить много пунктов (все шаги или концепции), "
        "назови кратко суть или предложи разбирать их по одному, чтобы не перегружать человека.\n"
    ...
    payload = {
        ...
        "temperature": 0.7,
        "max_tokens": 800  # Увеличили лимит, чтобы модель успевала отвечать на объемные запросы
    }
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=payload) as response:
                ...
                # Если сервер вернул не 200, логируем для отладки, но даем мягкий ответ
                error_body = await response.text()
                print(f"Groq API Error [{response.status}]: {error_body}")
                return "Брат, сейчас чуть-чуть не получилось обработать такой большой запрос. Давай начнем с какого-то одного шага или концепции?"
                
    except Exception as e:
        print(f"Exception during request: {e}")
        return "Произошел небольшой технический сбой. Главное — оставайся трезвым, напиши мне еще раз."